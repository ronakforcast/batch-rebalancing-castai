...
# Usage: ./script.sh <cluster_id> <api_key> <batch_size> [zone] [batch_percentage]

if [ $# -lt 3 ]; then
    echo "Usage: $0 <cluster_id> <api_key> <batch_size> [zone] [batch_percentage]"
    echo "  If batch_percentage is provided (e.g., 20 for 20%), it overrides batch_size"
    exit 1
fi
...
# Set batch size based on percentage if provided
if [ -n "$BATCH_PERCENTAGE" ]; then
    CALCULATED_BATCH_SIZE=$(( (TOTAL_NODES * BATCH_PERCENTAGE + 50) / 100 ))
    # Ensure at least 1 node per batch
    if [ $CALCULATED_BATCH_SIZE -lt 1 ]; then
        CALCULATED_BATCH_SIZE=1
    fi
    BATCH_SIZE=$CALCULATED_BATCH_SIZE
    echo "Using percentage-based batch size: ${BATCH_PERCENTAGE}% of $TOTAL_NODES nodes = $BATCH_SIZE nodes per batch"
else
    echo "Using fixed batch size: $BATCH_SIZE nodes per batch"
fi
